...
export function Landing() {
  ...
  return (
    <div className="min-h-screen bg-app text-body">
      <header className="sticky top-0 z-40 border-b border-line bg-app backdrop-blur">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          <Link to="/" className="flex items-center gap-2 text-lg font-bold text-strong">
          ...
          <nav className="hidden items-center gap-6 text-sm text-muted md:flex" aria-label="Main">
            <Link to="/marketplace" className="transition-colors hover:text-strong">
              Marketplace
            </Link>
            <Link to="/tournaments" className="transition-colors hover:text-strong">
              Tournaments
            </Link>
            <Link to="/leaderboard" className="transition-colors hover:text-strong">
              Leaderboard
            </Link>
          </nav>
      ...
      <main>
        <section className="relative overflow-hidden">
          ...
          <div className="relative mx-auto max-w-7xl px-4 py-24 text-center sm:px-6 lg:py-32">
            ...
            <h1 className="mx-auto max-w-3xl text-4xl font-bold leading-tight text-strong sm:text-6xl">
              Own your skill. <span className="bg-gradient-to-r from-brand-500 to-violet-500 bg-clip-text text-transparent">Earn on-chain.</span>
            </h1>
        ...
        <section className="border-y border-line bg-panel">
          <div className="mx-auto grid max-w-7xl grid-cols-2 gap-6 px-4 py-10 text-center sm:px-6 md:grid-cols-4">
            <div>
              <p className="text-3xl font-bold text-strong">48k</p>
              <p className="mt-1 text-sm text-muted">players on-chain</p>
            </div>
            <div>
              <p className="text-3xl font-bold text-strong">1.2k</p>
              <p className="mt-1 text-sm text-muted">tournaments hosted</p>
            </div>
            <div>
              <p className="text-3xl font-bold text-strong">$2.4M</p>
              <p className="mt-1 text-sm text-muted">prizes paid out</p>
            </div>
            <div>
              <p className="text-3xl font-bold text-strong">12</p>
              <p className="mt-1 text-sm text-muted">supported games</p>
            </div>
          </div>
        </section>

        <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6">
          <h2 className="text-center text-3xl font-bold text-strong">Pick your game</h2>
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {games.map((game) => (
              <Link
                key={game.name}
                to="/tournaments"
                className="group rounded-xl border border-line bg-panel p-5 transition-colors hover:border-brand-700"
              >
                <div
                  className={`mb-4 h-28 rounded-lg bg-gradient-to-br ${game.gradient} opacity-80 transition-opacity group-hover:opacity-100`}
                />
                <p className="font-semibold text-strong">{game.name}</p>
                <p className="mt-0.5 text-sm text-muted">{game.genre}</p>
              </Link>
            ))}
          </div>
        </section>
        ...
        <section className="bg-panel py-20">
          <div className="mx-auto max-w-7xl px-4 sm:px-6">
            <h2 className="text-center text-3xl font-bold text-strong">Built for competitive players</h2>
            <div className="mt-10 grid gap-6 md:grid-cols-3">
              {features.map((feature) => (
                <Card key={feature.title}>
                  <h3 className="text-lg font-semibold text-strong">{feature.title}</h3>
        ...
        <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6">
          ...
          <div className="mt-10 grid gap-8 md:grid-cols-3">
            {steps.map((item) => (
              <div key={item.step} className="relative rounded-xl border border-line bg-panel p-6">
                <span className="text-3xl font-black text-brand-500/60">{item.step}</span>
                <h3 className="mt-3 font-semibold text-strong">{item.title}</h3>
      ...
      <footer className="border-t border-line">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-4 py-8 text-sm text-muted sm:flex-row sm:px-6">
          ...
          <div className="flex gap-6">
            <a href="https://github.com/roastellar-org" target="_blank" rel="noreferrer" className="transition-colors hover:text-strong">
              GitHub
            </a>
            <a href="#" className="transition-colors hover:text-strong">
              Docs
            </a>
            <a href="#" className="transition-colors hover:text-strong">
              Terms
